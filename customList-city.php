...
<h3>Enter the skill and the City and find the companies in that city.</h3>
<form name="search-form" 
      id="search-form" 
      action="customList-city.php" 
      class="form-search"  method="post">
      <input   type="text" value="" placeholder="<?php print $keyword ?>" id="keyword" name="keyword"  style="padding-right:30px; float:left; " autocomplete="off"> 

<input   type="text" value="" placeholder="<?php print $city ?>" id="city" style="padding-right:30px; float:left; " name="city" autocomplete="off">  
...
SELECT `companies`.`idcompanies`,
    ...
    `companies`.`city`,
    `companies`.`state_abbriviation`,
    `companies`.`searchString`,
    `companies`.`postingCount`
FROM `lovedjobs`.`companies`
where 1=1  ".$citySqlTest ."  ".$argJob."   ORDER BY postingCount DESC ;
";

//SELECT 	description, Postdate,jobtitle, url, company, city, state, raw_Indeed.searchstring 	FROM raw_Indeed where 1=1  ".$citySqlTest ."  ".$argJob."   ORDER BY Postdate DESC LIMIT 1000";
...
{    
//&orig=GLHD&rsid=65525291465163952443&pageKey=voltron_job_search_internal_jsp&trkInfo=tarId%3A1465173383737&search=Search
echo  "<li class=\"detailBox\"><a target=\"_blank\" href=\"https://www.linkedin.com/vsearch/c?type=companies&keywords=".$row[companyName]."&search=Search\" >".substr($row[companyName],'0','50')."  (".$row[postingCount].") <img style=\"padding-left:10px\" src=\"assets\\web.png\" width=\"12\" heigth=\"12\">
</li>"   ;
}
print "</ul>"; 
}		
