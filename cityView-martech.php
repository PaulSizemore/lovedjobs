...
writing-mode: lr-tb;
    ...
    font: normal normal normal 8px/8px "Trebuchet MS", Helvetica, sans-serif; text-overflow:clip;
}
...
print "<H1>Cities with population over ".number_format($populationSize).".</H1><div class=\"skillLabelContainer\" ><div  class=\"cityNameContainer\">City Name</div>";
								...
								FROM `skills` WHERE skillCategory = \"martech\"   Order by displayOrder"; 	
...
{     $cityPopulation = $row[population]; 
								...
								$sqlMarTech = "SELECT `skills`.`idskills`, `skills`.`skillName`, `skills`.`skillCategory`, `skills`.`skillSearchTerm` FROM `skills` WHERE skillCategory = \"martech\"  Order by displayOrder"; 	
								$resultMarTech= $conn->query($sqlMarTech); 				
								if ($resultMarTech->num_rows > 0) {  	
